...
function CalendarPage() {
  ...
  const create = useMutation({
    mutationFn: () => {
      ...
      return createFn({
        data: { title, location: location || null, starts_at: dt.toISOString() },
      });
    },
    onSuccess: () => {
    ...
    <div className="px-5 pt-12 pb-8">
      <header className="mb-6 flex justify-between items-end">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="p-2 border border-ink/15"
          >
      ...
      <div className="grid grid-cols-7 mb-1 gap-px">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div
            key={i}
            className="text-[10px] uppercase tracking-widest text-ink/40 text-center py-1"
          >
            {d}
          </div>
        ))}
      ...
      <div className="grid grid-cols-7 gap-px bg-ink/5 border border-ink/10 mb-8">
        {days.map((d) => {
          ...
          return (
            ...
            >
              <span
                className={`text-xs font-medium ${isToday && !isSel ? "text-accent" : ""}`}
              >
                {format(d, "d")}
              </span>
              {has && (
                <span
                  className={`size-1 rounded-full ${isSel ? "bg-paper" : "bg-accent"}`}
                />
              )}
            </button>
          );
      ...
      <section>
        <div className="flex justify-between items-center mb-3">
          ...
          <button
            ...
            className="size-9 bg-ink text-paper flex items-center justify-center"
          >
